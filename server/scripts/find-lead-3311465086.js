...
const PHONE = '3311465086';

(async () => {
  await mongoose.connect(process.env.DATABASE);

  const leads = await Lead.find({
    numeroTelefono: { $in: [PHONE, `+39${PHONE}`, `39${PHONE}`] }
  ...
  const logs = await DeepagentLog.find({ userPhone: { $in: [PHONE, `+39${PHONE}`, `39${PHONE}`] } }).sort({ receivedAt: 1 }).lean();
