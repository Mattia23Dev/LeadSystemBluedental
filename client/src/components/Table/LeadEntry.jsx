...
export default function LeadEntry({ secref, id, index, data, handleModifyPopup, handleModifyPopupEsito, handleRowClick, campagna, etichette, selezionOrientatore }) {
  ...
  if (data)
    return (
      <div key={index} className={data.appVoiceBot ? "leadentry voicebot-lead" : data.appDate && data.appDate !== "" ? "leadentry mgm-lead" : data.campagna.includes("GFU") ? "leadentry gfu-lead" : "leadentry"}
        ref={ref}
        draggable="true"
      ...
      >
        ...
        {data.appVoiceBot && data.punteggio != null && 
        <div className='punteggio'>
            {String(data.punteggio) + "/2"}
        </div>}
      </div>
    )
  return <></>
